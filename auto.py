...
class Car:
    def __init__(self):
        ...
        self.target_city = None  # New attribute for target city
      
    def set_target_city(self, city_name):
        """Set the target city for the car to stop at."""
        if city_name in self.tag_id_to_city_name.values():
            self.target_city = city_name
            print(f"Target city set to {city_name}")
        else:
            print(f"City {city_name} not recognized. The car will stop at all cities by default.")
    ...
    def run(self):
        ...
        if curr_id != None and self.lst_tag != curr_id and self.target_city != None: # we assume the same id will not show up twice in a row
            if curr_id != self.target_city:
                self.lst_tag = curr_id
                time.sleep(0.5)
                self.motor_control.stop(6) # car will stop & Wait for "input" seconds
        ...
        elif curr_id != None and self.lst_tag != curr_id and self.target_city == None:
            self.lst_tag = curr_id
            time.sleep(0.5)
...
def user_input_process(conn):
    while True:
        target_city = input("Enter the target city name (leave blank for all cities): ")
        conn.send(target_city)  # Send the input to the main process
      
if __name__ == "__main__":
    parent_conn, child_conn = Pipe()
    input_process = Process(target=user_input_process, args=(child_conn,))
    input_process.start()
    ...
    try:
        while True:
            if parent_conn.poll():  # Check if there is new input
                new_city = parent_conn.recv()  # Receive the new city name
                car.set_target_city(new_city)
            car.run()  # Run a single cycle of car's operation
